...
//			break;
		...
		default:
			...
			Error("Unknown control type %d in object %i, sig/type/id = %i/%i/%i",obj->control_type, static_cast<short int>(obj), obj->signature.get(), obj->type, obj->id);
	...
	range_for (const auto i, highest_valid(Objects))
	{
		...
		if (o->type != OBJ_NONE)
			if (update_object_seg(o) == 0) {
				...
				con_printf(CON_URGENT, "Object %u claims segment %u, but has position {%i,%i,%i}; moving to {%i,%i,%i}", static_cast<short int>(o), o->segnum, pos.x, pos.y, pos.z, o->pos.x, o->pos.y, o->pos.z);
			}
	}
}
